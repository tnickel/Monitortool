...
public class AnnotationDemo1 extends ApplicationFrame
{
	...
	private static XYSeriesCollection createDataset()
	{
		...
		try
		{
			...
			while (data != null)
			{
				...
				float p97 = Float.parseFloat(data.substring(212, data.length())
						.trim());
				if (sex == 1)
				{
					s3.add(age, p3);
	...
	private static JFreeChart createChart(XYDataset dataset)
	{
		JFreeChart chart = ChartFactory.createXYLineChart(null,
				"Age in Months", "Weight (kg)", dataset,
				PlotOrientation.VERTICAL, true, true, false);
		TextTitle t1 = new TextTitle("Growth Charts: United States", new Font(
	...
	/**
	 * Starting point for the demonstration application when it is run as a
