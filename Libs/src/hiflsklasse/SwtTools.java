...
public class SwtTools
{
	...
	/*static public void baueUserListe(List list, HashSet<String> usermenge,
			HashMap<Integer, String> hmap, UserDB udb)
	{
		//füllt ie userliste die auf der rechten seite angezeigt wird
		//list: die userliste die auf der rechten seite angzeigt wird wird gefüllt
		//usermenge: hier sind alle user drin die links gepostet haben
		//hmap: zuordnung von position zu usernamen
		Iterator<String> i = usermenge.iterator();
		int pos = 0;
		while (i.hasNext())
		{
			String unam = i.next().toString();
			UserDbObj udbo = udb.getUserobj(unam);
			list.add(unam+"["+udbo.getRang()+"]"+"("+udbo.getManbewertung()+")");
			hmap.put(pos, unam);
			pos++;
		}
	}*/
