...
public class Tableview extends TableViewBasic
{
	...
	public void init(Display dis, Brokerview bview, Tradefilter tf,
			Table table, ProgressBar pb, int forceflag)
	{
		progressbar_glob = pb;
		table2_glob = table;
		display_glob = dis;
			...
			int showflag)
	{
		...
		if (new File(fnam1).exists() == true)
			readTradesFile(tl, fnam1, nocanceledflag, showopenorders, normflag,
					mc, tf_glob);
		//wenn kein history.txt und kein history_transfer.txt
		//dann gibt es nix zu lesen
		if ((new File(fnam2).exists() == false)
				&& (new File(fnam5).exists() == false))
		{
		...
		if (tf_glob.isLoadexpired()==true)
		{
			...
			if (hist_exp_f.exists() == true)
			{
				...
				readTradesFile(tl, fna, nocanceledflag, showopenorders,
						normflag, mc, tf_glob);
			}
		}
		...
		if (maxdate.length() < 2)
		{
			// Mbox.Infobox("Warning metatrader<"+mc.getBrokername()+"> hat noch keine EA´s -->Stop");
			return;
		}
		...
		if (alter > mc.getValiditydays() - 5)
		{
			...
			mc.setWarningflag(1);
			if (nostopflag == 0)
				Mbox.Infobox("Brokeraccount <" + mc.getBrokername()
						+ "> ending in less than 5 days");
			Mlist.add("Brokeraccount <" + mc.getBrokername()
					+ "> ending in less than 5 days", 1);
		} else
		{
			// warningflag wieder löschen
	...
	public void ShowProfitTable()
	{
		...
		for (int j = 0; j < profanzahl; j++)
		{
			...
			// zeile
			if ((gew10 < 0) && (gew30 < 0) && (gewall < 0)
					&& (prof.getGestrades() > 5) && (ea.getAuto() == 1))
			{
			...
			// inst
			item.setText(6, String.valueOf(ea.getInst()));
			// auto
	...
	public Tradeliste showEinzelTradeliste(Tradeliste einzeltradeliste,
			Display dis, Table table, Marklines marklines,
...

	{
		...
		// sideflag: 0, dann ist dies die linke seite der tabelle, 1 = rechte seite der tabelle
		...
		for (int i = 0, j = 0; i < zeilcount; i++)
		{
			...
			// if ((tr.getBroker().equals(broker)) && (tr.getMagic() == magic))
			{
				...
				item.setText(6, SG.kuerzeFloatstring(
						String.valueOf(tr.getOpenprice()), 5));
				item.setText(7, tr.getClosetime());
				item.setText(8, SG.kuerzeFloatstring(
						String.valueOf(tr.getCloseprice()), 5));
				item.setText(9,
						SG.kuerzeFloatstring(String.valueOf(tr.getProfit()), 2));

				String pipval = String.valueOf(Math.abs(tr.getCloseprice()
						- tr.getOpenprice()) * 10000);
				// die nachkommastellen entfernen
				if (pipval.contains("."))
					pipval = pipval.substring(0, pipval.indexOf("."));

				item.setText(10, pipval);
				item.setText(11, SG.kuerzeFloatstring(
						String.valueOf(einzeltradeliste.get_tsumx(i)), 2));
				item.setText(12, SG.kuerzeFloatstring(
						String.valueOf(einzeltradeliste.calc_gdx(i, 5)), 2));
	...
	public ArrayList<Tradeliste> buildAllTradeliste(Tradefilter tf,
			int maxprofanz)
	{
	...
	private void ToggleOnOffEa(int magic, String selbroker)
	{
		...
		if (demobroker == null)
		{
			Mlist.add("W: realbroker<" + realbroker
					+ "> don´t have a con. demobroker", 1);
			eal.toggleOn(magic, demobroker);
		}
		...
		if (ret == 1)
		{
			Mlist.add("switch ON EA<" + magic + "> on realbroker<" + realbroker
					+ ">", 1);
			// EA wurde eingeschaltet
			networker_glob
					.RealBroSwitchOnEa(brokerview_glob, magic, realbroker);
		} else if (ret == 0)
		{
			Mlist.add("switch OFF EA<" + magic + "> on realbroker<"
					+ realbroker + ">", 1);
			// EA wurde ausgeschaltet
			networker_glob.RealBroSwitchOffEa(brokerview_glob, magic,
					realbroker);
		} else
	...
	public String getSourcedir(String selbroker)
	{
		...
		return (brokerview_glob.getMetaconfigByBrokername(selbroker)
				.getMqlquellverz());
	...
	public void lifecheck(Metaconfig mc)
	{
		...
		if (Tools
				.CheckDateIsOlder(sdf.format(histnam.lastModified()), 0, 15, 0))
			Mbox.Infobox("Broker" + mc.getBrokername()
					+ "not working last access="
	...
	public String calcTradepicture(String headline, int forceflag,
			Tradeliste tlx)
	{
		String fnam = Profitanzeige.createProfitpicture(tlx, headline,
				forceflag);
		return (fnam);
	}
