...
public class StartMonitorWork
{
	...
	public void table2clicked(int index, String name,
			org.eclipse.swt.widgets.Button selonlyone, Table table)
	{
	...
	{
		MessageBox dialog = new MessageBox(display_glob.getActiveShell(),
				SWT.ICON_QUESTION | SWT.OK | SWT.CANCEL);
		dialog.setText("My info");
		dialog.setMessage("Do you really want to do delete the broker <"
				+ glob_selectedBrokerShare + ">");
		if (dialog.open() != 32)
			return;
	...
	public void installAutoEa()
	{
		...
		Metaconfig meRealconf = brokerview_glob
				.getMetaconfigByBrokername(meconf.getconnectedBroker());
		...
		Installer inst = new Installer();
		if (inst.InstalliereEinRealEaSystemFromDemobroker(tv_glob, prof,
				meconf, meRealconf, tv_glob.getEaliste()) == true)
		{
			Mbox.Infobox("EA magic<" + prof.getMagic() + "> installed on broker <"
					+ meconf.getconnectedBroker() + ">");
		} else
		{
			Mbox.Infobox("EA <" + prof.getMagic()
					+ "> installation ERROR on broker<"
					+ meconf.getconnectedBroker() + ">");
		}
	}
	...
	public void copyEA(String targetbroker)
	{
		...
		Metaconfig quell_meconf = brokerview_glob
				.getMetaconfigByBrokername(selectedProfitelem_glob.getBroker());
		String quellverz = quell_meconf.getMqlquellverz();
		Metaconfig ziel_meconf = brokerview_glob
				.getMetaconfigByBrokername(targetbroker);
	...
	public void ConfigEa()
	{
		...
		SwtConfigEa.startmain(display_glob, selectedProfitelem_glob, tv_glob,
				this, brokerview_glob.getAccounttype(glob_selectedBroker));
	}
	...
	public void exportOneEaAndTradelist()
	{
		...
		Metaconfig meconf = brokerview_glob.getMetaconfigByBrokername(trade
				.getBroker());
		// strname beinhaltet den ganzen Pfad
		String strname = ea.holeStrFilename(tv_glob.getEaliste(), meconf,
				trade.getComment(), 0);

		String tradelname = strname.substring(strname.lastIndexOf("\\") + 1)
				.replace(".str", ".tradeliste");
		...
		String mqlname = strname.substring(strname.lastIndexOf("\\") + 1)
				.replace(".str", ".mq4");
		...
		// den Ea kopieren
		System.out.println("kopiere ea von <" + eaquellname + "> nach <"
				+ zielname + ">");
	...
	public void exportTradelist()
	{
		...
		String fnam = view.fileRequester2(Display.getDefault(),
				"export tradelist", Rootpath.getRootpath(), SWT.SAVE);
	...
	public void importTradelist()
	{
		...
		MessageBox dialog = new MessageBox(display_glob.getActiveShell(),
				SWT.ICON_QUESTION | SWT.OK | SWT.CANCEL);
		dialog.setText("My info");
		dialog.setMessage("Do you really want to import new trades into broker <"
				+ glob_selectedBroker + ">");
		if (dialog.open() != 32)
			return;

		/*
		 * Tradeliste eatradeliste = tv_glob.buildTradelisteAllSel(1); if
		 * (eatradeliste == null) return;
		 */
		Viewer view = new Viewer();
		String fnam = view.fileRequester2(Display.getDefault(),
				"import tradelist", Rootpath.getRootpath(), SWT.READ_ONLY);
	...
	public void showBacktest(boolean forceflag)
	{
		...
		{
			filename = ea.holeStrFilename(tv_glob.getEaliste(), meconf,
					tv_glob.getComment(ea.getMagic()), 1);
		}
		...
		if ((forceflag == false)
				&& (FileAccess.FileAvailable(filename) == true))
			showBacktestJPG(filename);
		// 2)falls gif bild da ist
		else if ((forceflag == false)
				&& (FileAccess.FileAvailable(filename.replace(".jpg", ".gif")) == true))
	...
	private boolean showBacktestXml(String storefilename)
	{
		...
		Ea ea = tv_glob.getEaliste().getEa(pro.getMagic(), pro.getBroker());
		String headline = pro.getBroker() + ":" + pro.getMagic() + ":"
				+ ea.getInfo();

		Profitanzeige profanz = new Profitanzeige("Gewinnverlauf",
				eatradeliste, headline, storefilename);
	...
	public void setInstfromDemobroker()
	{
		...
		SwtSetInstfrom
				.mainx(brokerview_glob, profitmenge, tv_glob.getEaliste());
	}
