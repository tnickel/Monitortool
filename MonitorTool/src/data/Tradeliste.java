...
public class Tradeliste
{
	...
	{
		// clean Tradeliste
		if (transactionmenge != null)
			transactionmenge.clear();
		if (tradeliste != null)
			tradeliste.clear();
		if (summengewinne != null)
			summengewinne.clear();
		sumGewinn = 0.0;
		if (openzeitenmenge != null)
			openzeitenmenge.clear();
		if (closezeitenmenge != null)
			closezeitenmenge.clear();
	...
	public ArrayList<String> calcBrokerliste(int magic)
	{
		...
		// gesetzt, hier kann man nicht mehr die Trades eindeutig anhand der Magic
		// zuordnen
	...
	private Trade calcStrTrade(String tradezeile)
	{
		...
		// den ordertype holen
		int type = SG.get_zahl(tradezeile.substring(tradezeile.indexOf("<type>") + 6, tradezeile.indexOf("</type>")));
		...
		// den profit holen
		String profitstring = tradezeile.substring(tradezeile.indexOf("<pl>") + 4, tradezeile.indexOf("</pl>"));
		profit = Float.valueOf(profitstring);
		
		// die opentime holen
		String opentimestring = tradezeile.substring(tradezeile.indexOf("<openTime>") + 10,
				tradezeile.indexOf("</openTime>"));
		Long ot = Long.valueOf(opentimestring) * 1000 - (7200 * 1000) + (3600 * 1000);
		Date opentime = new Date(ot);
		
		// die closetime holen
		String closetimestring = tradezeile.substring(tradezeile.indexOf("<closeTime>") + 11,
				tradezeile.indexOf("</closeTime>"));
		Long ct = Long.valueOf(closetimestring) * 1000 - (7200 * 1000) + (3600 * 1000);
		Date closetime = new Date(ct);
		
		// den open-Price holen
		String openpricestring = tradezeile.substring(tradezeile.indexOf("<openPrice>") + 11,
				tradezeile.indexOf("</openPrice>"));
		
		// den close-Price holen
		String closepricestring = tradezeile.substring(tradezeile.indexOf("<closePrice>") + 12,
				tradezeile.indexOf("</closePrice>"));
		
		// symbol holen
		String symbol = "???";
		
		Trade tr = new Trade("0#" + type + "#" + openpricestring + "#" + closepricestring + "#0#0#" + profit + "#1#"
				+ sdf.format(opentime) + "#" + sdf.format(closetime) + "##499143#49648465" + "#" + symbol + "#"
				+ direction, "Alpari", false, 0);
	...
	private Trade calcCsvTrade(String line, int transnumber, String broker)
	{
		...
		Trade tr = new Trade("0#" + ordertype + "#" + parts[3] + "#" + parts[5] + "#0#0#" + parts[6] + "#0.1#"
				+ parts[2] + "#" + parts[4] + "#" + parts[7] + "#0#" + transnumber, broker, false, 0);
		return tr;
	}
	...
	private ArrayList<Trade> entferneTrades(Brokerview bview, ArrayList<Trade> tradelistetmp, Tradefilter tf)
	{
		...
		for (int opentradezaehl = 0, i = 0; i < anz; i++)
		{
			...
			{
				opentradezaehl++;
				Tracer.WriteTrace(20, "Info: <" + opentradezaehl + "> Offener Trade entfernt <" + tr.getComment()
						+ "> open<" + tr.getOpentime() + "> close<" + tr.getClosetime() + ">");
		...
		for (int i = 0; i < anz; i++)
		{
			...
			if (tr.getImportedcolor() == 0)
				...
				{
					Tracer.WriteTrace(20, "Info: Trade wird entfernt broker<" + broker + ">");
					tradelistetmp.remove(i);
					i = i - 1;
					anz--;
				}
	...
	public boolean addTradeElem(Trade elem)
	{
		...
		{
			// Tracer.WriteTrace(20, "I:broker<"+broker+"> trade schon drin
			// trans<"+trans+">");
			return false;
		}
	}
	...
	// Hier wird die Tradeliste auf platte gespeichert
	{
		...
		if (f2.length() * (1.2) < f1.length())
			Tracer.WriteTrace(20, "ERROR: Length protection error tradeliste<" + f1.length() + "> tradeliste_new<"
					+ f2.length() + ">");
	...
	public Double calc_gdx(int pos, int intervall)
	{
		...
		if (pos > summengewinne.size())
			Tracer.WriteTrace(10, "internal error pos=" + pos + "  > summenlistesize=" + summengewinne.size() + "");
	...
	public void kontrollausgabe()
	{
		...
		{
			Trade trade = tradeliste.get(i);
			inf.writezeile("i=" + i + " magic=" + trade.getMagic() + " closetime=" + trade.getClosetime() + " profit="
	...
	public void expandTL(String fnam, String broker)
	{
		...
		for (int i = 0; i < anz; i++)
		{
			...
			{
				Mlist.add("trade<" + tr.getBroker() + "><" + tr.getMagic() + "><" + tr.getOpentime() + ">imported");
				impcounter++;
			} else
			{
				Mlist.add("ImportError:<" + tr.getBroker() + "><" + tr.getMagic() + "><" + tr.getOpentime() + ">");
				badcounter++;
			}
		}
		...
		Mbox.Infobox("Imported trades into broker<" + broker + "> ok<" + impcounter + "> bad<" + badcounter + ">");
	...
	public void removeImported(String broker)
	{
		...
		{
			Trade tr = tradeliste.get(i);
			if ((tr.getImportedcolor() != 0) && (tr.getBroker().equalsIgnoreCase(broker)))
			{
				
				if (this.remTradeElem(tr))
				{
					Mlist.add("trade<" + tr.getBroker() + "><" + tr.getMagic() + "><" + tr.getOpentime() + ">removed");
					remcounter++;
					i = i - 1;
					anz = anz - 1;
				...
				{
					Mlist.add("RemoveError:<" + tr.getBroker() + "><" + tr.getMagic() + "><" + tr.getOpentime() + ">");
					badcounter++;
				}
			}
		}
		Mbox.Infobox("Removed trades from broker<" + broker + "> ok<" + remcounter + "> bad<" + badcounter + ">");
	}
			...
			Tradefilter tf, int forcesortflag)
	{
		...
		org.eclipse.swt.graphics.Color green = dis.getSystemColor(SWT.COLOR_GREEN);
		org.eclipse.swt.graphics.Color magenta = dis.getSystemColor(SWT.COLOR_MAGENTA);
		org.eclipse.swt.graphics.Color gray = dis.getSystemColor(SWT.COLOR_GRAY);
		...
		Swttool.baueTabellenkopfDispose(table, "Ind#Magic#Symb#Profit#Lots#Opentime#Closetime#Comment#Broker#Account");
		...
		for (int i = 0; i < zeilcount; i++)
		{
			...
			if (Tools.zeitdifferenz_stunden(Tools.get_aktdatetime_str(), closetime) < 16)
				item.setForeground(5, magenta);
			
			item.setText(3, SG.kuerzeFloatstring(String.valueOf(tr.getProfit()), 2));
	...
	public void calcMinMaxdate()
	{
		...
		for (int i = 0; i < anz; i++)
		{
			...
			try
			{
				...
				// System.out.println("dati closetime="+closetimestring+"
				// date="+date.toString());
	...
	public void calcMinMaxdateOpentime()
	{
		...
		for (int i = 0; i < anz; i++)
		{
			...
			try
			{
				...
				// System.out.println("dati opentimetime="+opentimestring+"
				// date="+date.toString());
			} catch (ParseException e)
			{
				// TODO Auto-generated catch block
	...
	public GewinnjahrStrategie calcGewinnjahrType1(int maxdd, int maxddproz, int maxddfromstart, int trailflag,
			int maxddtrail, int ntraildays)
	{
		...
		for (int i = 0; i < anz; i++)
		{
			...
			if (trailflag == 1)
			{
				...
				if (i < 9)
				{
					...
					if (aktgewin < maxverlusttrail)
					{
						Tracer.WriteTrace(20,
								"I:Trailing erreicht aktgewin=" + aktgewin + " tradeanz(" + i + "|" + anz + ")=");
						return (gewjahr);
					}
				}
				...
				else
				{
					...
					if (aktgewin < maxverlusttrail)
					{
						Tracer.WriteTrace(20, "I:Trailing erreicht aktgewin=" + aktgewin + " maxverlust="
								+ maxverlusttrail + " tradeanz(" + i + "|" + anz + ")=");
						return (gewjahr);
					}
				}
			}
			...
			if (maxddfromstart > 0)
			{
				...
				{
					Tracer.WriteTrace(20,
							"I:MaxddFromStart erreicht aktgewin=" + aktgewin + " tradeanz(" + i + "|" + anz + ")=");
					return (gewjahr);
				}
			}
			...
			if (maxdd > 0)
			{
				...
				if (dd > maxdd)
				{
					Tracer.WriteTrace(20, "I:Maxdd erreicht dd=" + dd + " tradeanz(" + i + "|" + anz + ")=");
					return (gewjahr);
				}
			}
			if (maxddproz > 0)
			{
				...
				if (ddproz > maxddproz)
				{
					Tracer.WriteTrace(20, "I:Maxdd proz erreicht dd%=" + ddproz + " tradeanz(" + i + "|" + anz + ")=");
					return (gewjahr);
				}
			}
		}
	...
	{
		/*
		 * In der Toolbox wird die Accountgröße angegeben, sagen wir mal 10.000€. Dann
		 * wird ein maximaler %DD festgelegt, der für das gesamte Portfolio gilt.
		 * Danijel hat diesen DD mit 30% festgelegt, er sollte aber in der Toolbox
		 * weiterhin variabel sein. Dann werden die 30% durch die Anzahl der Strategien
		 * geteilt und jeder EA bekommt sein persönliches Budget zum verpulvern.
		 * 
		 * 10.000€ * 30% = 3.000€ 3.000€/Anzahl der installierten EA´s (z.B. 100 Stück)
		 * = 30€ pro EA .
		 * 
		 * Diese 30€ gelten zu Anfang gesehen vom Startbudget aus und ab dem ersten
		 * Trade vom Allzeithoch der Equity-Kurve des EA. Sprich:
		 * 
		 * Trade 1. Der EA fängt bei Null an darf also bis -30€ fallen bis er
		 * abgeschaltet wird. Trade 2. Wenn der erste Trade +10€ gebracht hat dann
		 * steigt die Abschaltlinie von -30 auf -20€, es wird also immer das Allzeithoch
		 * -30€ (in diesem Beispiel) zum Abschalten genommen.
		 * 
		 * Ganz wichtig, die 30% werden nur einmal beim Start des Portfolio berechnet
		 * und das Budget an alle EA´s vergeben. Es gibt keine weitere prozentuale
		 * Anpassung wenn z.B. die Equity angestiegen ist oder EA´s abgeschaltet wurden!
		 */
		GewinnjahrStrategie gewjahr = new GewinnjahrStrategie();
		float maxgewin = 0;
		float aktgewin = 0;
	...
	private int calcmonat(String datum)
	{
		...
		String monzahl = datum.substring(datum.indexOf("-") + 1, datum.lastIndexOf("-"));
	...
	{
		openzeitenmenge = calcTradezeitenmenge(gmtdiff, 1, minutentolleranz);
		closezeitenmenge = calcTradezeitenmenge(gmtdiff, 0, minutentolleranz);
	}
