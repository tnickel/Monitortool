...
public class Tradefilter
{
	...
	boolean showrealaccounts = false;
	...
	public boolean isShowrealaccounts()
	{
		return showrealaccounts;
	}
	
	public void setShowrealaccounts(boolean showrealaccounts)
	{
		this.showrealaccounts = showrealaccounts;
	}
	...
	public boolean checkConditions(Profit prof)
	{
		...
		// mehr getraded hat
		if ((isForgetoldeas() == true) && (prof.calcDaysNeusterTrade() > GlobalVar.getForgetdays()))
			return false;
		...
		{
			// jetzt wird geprüft ob der ea mit einem realbroker verknüpft oder ein
			// Realbroker selbst ist
			if (this.showrealaccounts == true)
			{
	...
	public void init()
	{
		 ...
		 * inf.loadXST(); showopenorders=(Boolean) inf.loadXST(); forgetoldeas=(Boolean)
		 * inf.loadXST(); loadexpired=(Boolean) inf.loadXST(); minfavorit=(Integer)
		 * inf.loadXST(); maxfavorit=(Integer) inf.loadXST(); comment=new
		 * String((String) inf.loadXST());
		 * 
